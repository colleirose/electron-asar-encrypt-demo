// Taken from the Predator the Stealer source code leak, but reformatted
...
template <int F, size_t N, int K> struct XorString;
...
template <int N> struct RandomGeneratorXor {
private:
...
template <> struct RandomGeneratorXor<0> {
...
template <size_t N, int K> struct XorString<0, N, K> {
private:
  const char _key;
...
template <size_t N, int K> struct XorString<1, N, K> {
private:
  const char _key;
  std::array<char, N + 1> _encrypted;
  constexpr char enc(char c) const { return c + (_key % 16); }

  char dec(char c) const { return c - (_key % 16); }
...
template <size_t N, int K> struct XorStringW {
private:
  std::array<wchar_t, N + 1> _encrypted;
  constexpr wchar_t enc(wchar_t c) const { return ~c; }

  wchar_t dec(wchar_t c) const { return ~c; }

public:
  template <size_t... Is>
  constexpr __forceinline XorStringW(const wchar_t *str,
...
volatile constexpr __forceinline int getInt(volatile int a) { return a; }

#ifdef RELEASE_BUILD
#define XorStr(s)                                                              \
  (XorCompileTime::XorString<XorCompileTime::RandomInt<__COUNTER__, 2>::value, \
                             sizeof(s) - 1, __COUNTER__>(                      \
       s, std::make_index_sequence<sizeof(s) - 1>())                           \
       .decrypt())
#define XorStrW(s)                                                             \
  (XorCompileTime::XorStringW<sizeof(s) / 2 - 1, __COUNTER__>(                 \
       s, std::make_index_sequence<(sizeof(s) / 2 - 1)>())                     \
       .decrypt())
#define XorIntP(n, b)                                                          \
  (XorCompileTime::getInt(XorCompileTime::RandomInt<b, 2048>::value ^ n) ^     \
   XorCompileTime::RandomInt<b, 2048>::value)
